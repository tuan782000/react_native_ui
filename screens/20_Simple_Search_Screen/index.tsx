...
const SimpleSearchScreen = () => {
  ...
  // Xử lý search
  // lý do sử dụng useMemo ghi nhớ danh sách users
  // khi giá trị phụ thuộc input "đang lưu trong state" thay đổi - thì mới kích hoạt callback bên trong.
  // còn lại thì không chạy tránh re-render không cần thiết
  const filteredRows = useMemo(() => {
    const rows = [];
    const query = input.toLowerCase(); // nội dung người dùng nhập input - tất cả nội dung sẽ viết thường

    // dùng for chạy qua từng item trong users
    for (const item of users) {
      const nameIndex = item.name.toLowerCase().search(query); // search hàm có sẵn js cung cấp so sánh name đã được viết thường với query người dùng nhập đã được viết thường - nếu tìm thấy item trả về item đó - không tìm thấy trả về -1
      const phoneIndex = item.phone.toLowerCase().search(query);

      if (nameIndex !== -1 || phoneIndex !== -1) {
          // những item != -1 sẽ được đẩy vào mảng rows
          // bằng cách thêm nó vào cuối mảng. Trải mảng rows toàn bộ item, thêm nó vào cuoois mảng
        rows.push({
          ...item,
        //   index: nameIndex,
          index: nameIndex !== -1 ? nameIndex : phoneIndex, // Dùng chỉ số nào tìm thấy trước
        });
      }
    }

    // Thằng nào tìm thấy trước thì lên đầu, thằng nào tìm thấy sau đứng sau
    return rows.sort((a, b) => a.index - b.index); // trước khi trả về kết quả sắp xếp lại theo thứ tự
  }, [input]);
